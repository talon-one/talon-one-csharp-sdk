...
namespace TalonOneSdk.Model
{
    ...
    public partial class Audience : IValidatableObject
    {
        ...
        [JsonConstructor]
        public Audience(long accountId, long id, DateTime created, string name, Option<bool?> sandbox = default, Option<string> description = default, Option<List<long>> subscribedApplicationsIds = default, Option<string> integration = default, Option<string> integrationId = default, Option<bool?> createdIn3rdParty = default, Option<DateTime?> lastUpdate = default)
        {
            AccountId = accountId;
            Id = id;
            Created = created;
    ...
    public partial class AudienceJsonConverter : JsonConverter<Audience>
    {
        ...
        public override Audience Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!accountId.IsSet)
                throw new ArgumentException("Property is required for class Audience.", nameof(accountId));

            if (!id.IsSet)
                throw new ArgumentException("Property is required for class Audience.", nameof(id));

            if (!created.IsSet)
                throw new ArgumentException("Property is required for class Audience.", nameof(created));
            ...
            return new Audience(accountId.Value.Value, id.Value.Value, created.Value.Value, name.Value, sandbox, description, subscribedApplicationsIds, integration, integrationId, createdIn3rdParty, lastUpdate);
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, Audience audience, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            writer.WriteNumber("id", audience.Id);

            writer.WriteString("created", audience.Created.ToString(CreatedFormat));

            writer.WriteString("name", audience.Name);

            if (audience.SandboxOption.IsSet)
                writer.WriteBoolean("sandbox", audience.SandboxOption.Value.Value);

            if (audience.DescriptionOption.IsSet)
                writer.WriteString("description", audience.Description);

            if (audience.SubscribedApplicationsIdsOption.IsSet)
            {
                writer.WritePropertyName("subscribedApplicationsIds");
                JsonSerializer.Serialize(writer, audience.SubscribedApplicationsIds, jsonSerializerOptions);
            }
            if (audience.IntegrationOption.IsSet)
                writer.WriteString("integration", audience.Integration);

            if (audience.IntegrationIdOption.IsSet)
                writer.WriteString("integrationId", audience.IntegrationId);

            if (audience.CreatedIn3rdPartyOption.IsSet)
                writer.WriteBoolean("createdIn3rdParty", audience.CreatedIn3rdPartyOption.Value.Value);

            if (audience.LastUpdateOption.IsSet)
                writer.WriteString("lastUpdate", audience.LastUpdateOption.Value.Value.ToString(LastUpdateFormat));
        }
    }
}
